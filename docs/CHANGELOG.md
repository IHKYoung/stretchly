...
- 父仓库同步纳入官网 latest 下载解析、失败时 pinned 回退及对应生产核验记录；本轮不执行新的部署或发布。
...
- 将根 `package.json`、两份 npm lockfile、desktop package、Cargo 与 Tauri 版本真源统一提升到 `0.1.5`；本轮只形成本地版本提交，不包含安装包、tag、push 或 GitHub Release。
