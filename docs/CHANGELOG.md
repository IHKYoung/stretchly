...
- 智能提醒到点后等待自然空档时，tray title 现在显示由 Tauri 运行时提供的最长等待倒计时；这条临时反馈即使关闭常规计划倒计时也会保留，pause/focus/DND/app exclusion 等 blocker 仍按既有优先级处理。
- break ideas 新增批次 manifest、写作维护说明与内容治理测试；生成器现在会在构建 registry 前检查 official locale 的 ID/shape parity、批次连续性、重复正文、长度预算、类别交错和易产生模板化/无依据表达的禁用短语。

### 调整

- 保留原有提示语，并将 `zh-CN / zh-TW / en` 的微休息从 `364` 条扩到 `728` 条、完整休息从 `244` 条扩到 `488` 条；新增内容交错覆盖工作调侃、身体观察、轻量健康知识、情绪关照、日常场景与小任务，完整休息采用更长的口语段落。
- 自定义节奏页改用“每隔 / 持续 / 每 N 个提醒周期”的完整句式，命中 preset 时只显示明确的“自定义”输入入口，并补充完整休息折算分钟与周期说明。
- 设置页继续使用霞鹜文楷，并为页标题、分区标题、设置项、摘要、详情和单位建立统一字号、字重与行高层级。
- smart 模式下由 tray/快捷键提前提醒时，运行时文案不再承诺“立即开始”，避免把等待自然空档误报成已经进入 break。
- 父仓库同步纳入官网 latest 下载解析、失败时 pinned 回退及对应生产核验记录；`v0.1.5` 发布后固定回退同步到本版 Apple Silicon 安装包。
- 根 `.gitignore` 将历史翻译实验统一收口到 `scratch/locale-translations/`，保留 `scratch/README.md` 说明维护真源与临时文件边界。
- 将根 `package.json`、两份 npm lockfile、desktop package、Cargo 与 Tauri 版本真源统一提升到 `0.1.5`，并发布签名、公证的 macOS Apple Silicon 安装包。
- 根据用户反馈重构桌面端设置页的信息架构：默认页保留三档休息节奏方案与常用设置，并直接提供自定义节奏、提醒与延后、休息外观与声音、自动适应、系统五个主题入口；移除“详细设置”中转层，所有主题页直接返回默认页，既有非标准节奏会保留并显示为自定义。
- 依据 AOA、HSE、NIOSH 与微休息研究重新校准三档节奏：少打扰、均衡、多活动分别采用 `30m/20s`、`20m/20s`、`10m/30s` 微休息，并统一约每小时安排 `5m` 完整休息；新安装默认采用均衡档，已有持久化节奏不自动迁移。
- 将 long break 的中文用户术语统一为“完整休息”，并把完整休息提示语的轮换停留从完整呈现后 `30s` 延长到 `60s`；微休息仍固定只显示一条提示语。

### 文档

- 完成 `v0.1.5` 发布审计：记录 Apple app/DMG 双重公证、GitHub Release 远端下载 hash 回验，以及官网 latest/fixed 下载路径与 Vercel 生产部署证据。
