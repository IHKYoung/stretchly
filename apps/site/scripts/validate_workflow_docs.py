...
FIELD_ALIASES: Dict[str, Sequence[str]] = {
    ...
    "Delegation Policy": ("委派策略", "Delegation 策略"),
    "Automatic Delegation Triggers": ("自动委派触发条件", "自动 delegation 触发条件"),
    "Orchestrator Execution Profile": ("Orchestrator 执行画像",),
    "Execution Mode Policy": ("执行模式策略",),
...
def _normalize_status_token(raw: str) -> str:
    normalized_raw = _normalize_value_token(raw)
    if normalized_raw in {"n/a", "na"}:
        return "n/a"
    tokens = _split_value_tokens(raw)
    if len(tokens) >= 2 and tokens[0] == "n" and tokens[1] == "a":
        return "n/a"
    return tokens[0] if tokens else _normalize_value_token(raw)


def _is_yes_token(raw: str) -> bool:
    return _normalize_status_token(raw) in {"yes", "true", "required"}
...
def _validate_task_level_semantics(repo_root: Path, tid: str, plan_block: str, log_block: str) -> List[ValidationError]:
    ...
    if plan_path.exists():
        ...
        operator_approval_required = _field_value(plan_content, "operator_approval_required") or ""
        touches_running_service = _field_value(plan_content, "touches_running_service") or ""
        destructive_operations = _normalize_status_token(_field_value(plan_content, "destructive_operations") or "")
        high_risk_approval_required = any(
            (
                _is_yes_token(operator_approval_required),
                _is_yes_token(touches_running_service),
                destructive_operations not in {"", "none", "no", "n/a", "na"},
            )
        )
    ...
    if level in {"moderate", "complex"}:
        ...
        if high_risk_approval_required and approval_needed != "yes":
            errors.append(
                ValidationError(
                    f"specs:{tid}:plan.md",
                    f"`{level}` 任务的 `Execution Safety Block` 已标记高风险动作，plan.md 必须声明 `Approval needed: yes`。",
                )
            )
        if not high_risk_approval_required and approval_needed not in {"no", "n/a", "na"}:
            errors.append(
                ValidationError(
                    f"specs:{tid}:plan.md",
                    f"`{level}` 任务若不存在高风险动作，应在 plan.md 中声明 `Approval needed: no`。",
                )
            )
        if _status_requires_completed_specs(plan_status_raw):
            if high_risk_approval_required and (not approved_state or approved_state in PLACEHOLDER_TOKENS):
                errors.append(
                    ValidationError(
                        f"specs:{tid}:plan.md",
                        f"`{level}` 高风险任务进入非 INIT 状态后，`Approved` 不能继续为占位值。",
                    )
                )
            if not high_risk_approval_required and approved_state not in {"no", "n/a", "na"}:
                errors.append(
                    ValidationError(
                        f"specs:{tid}:plan.md",
                        f"`{level}` 低风险任务进入非 INIT 状态后，`Approved` 应明确写为 `N/A` 或 `no`，避免与用户审批混淆。",
                    )
                )
