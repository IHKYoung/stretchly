...
describe('pauseBreaksShortcut', () => {
  ...
  describe('onShortcut', () => {
    ...
    describe('skipToNextScheduledBreakShortcut', () => {
      it('skips to next scheduled break', () => {
        ...
        expect(log.info).toHaveBeenCalledWith('Pauza: skipping to next scheduled break by shortcut')
    ...
    describe('resetBreaksShortcut', () => {
      it('rest breaks', () => {
        ...
        expect(log.info).toHaveBeenCalledWith('Pauza: resetting breaks by shortcut')
    ...
    describe('skipToNextLongBreakShortcut', () => {
      it('skips to next scheduled long break', () => {
        ...
        expect(log.info).toHaveBeenCalledWith('Pauza: skipping to next Long break by shortcut')
        expect(skipToBreak).toHaveBeenCalled()
        expect(pauseBreaks).not.toHaveBeenCalled()
      })
    })
    ...
    describe('skipToNextMiniBreakShortcut', () => {
      it('skips to next scheduled microbreak', () => {
        ...
        expect(log.info).toHaveBeenCalledWith('Pauza: skipping to next Mini break by shortcut')
        expect(skipToMicrobreak).toHaveBeenCalled()
        expect(pauseBreaks).not.toHaveBeenCalled()
      })
    })
  ...
  describe('setupBreak', () => {
    it('should register a shortcut and call pauseBreaks with the correct interval', () => {
      ...
      expect(pauseBreaks).toHaveBeenCalledWith(30 * 60 * 1000)
      expect(log.info).toHaveBeenCalledWith('Pauza: pauseBreaksFor30MinutesShortcut registration successful (Ctrl+Shift+P)')
    })
    ...
    it('should log a warning if shortcut registration fails', () => {
      ...
      expect(log.warn).toHaveBeenCalledWith('Pauza: pauseBreaksFor30MinutesShortcut registration failed')
    })
  })
  ...
  describe('registerBreakShortcuts', () => {
    ...
    it('should skip empty shortcuts', () => {
      ...
      expect(log.info).toHaveBeenCalledWith('Pauza: pauseBreaksFor30MinutesShortcut registration successful (key)')
    })
  })
})
