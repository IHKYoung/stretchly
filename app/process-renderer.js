...
window.onload = async (e) => {
  window.pauza.onPlaySound((file, volume) => {
    __electronLog.info(`Pauza: playing audio/${file}.wav (volume: ${volume})`)
    const audio = new Audio(`audio/${file}.wav`)
    audio.volume = volume
    audio.play()
  })

  window.pauza.onShowNotification(async (text, silent) => {
    __electronLog.info(`Pauza: showing notification "${text}" (silent: ${silent})`)
  ...
  window.pauza.onCheckVersion(async (oldVersion, notify, silent) => {
      ...
      new VersionChecker()
        ...
        .then(async version => {
          if (version) {
            ...
            __electronLog.info(`Pauza: checking for new version (local: ${oldVersion}, remote: ${cleanVersion})`)
            if (await window.semver.valid(cleanVersion) && await window.semver.gt(cleanVersion, oldVersion)) {
              ...
              window.pauza.updateTray()
              if (notify) {
                notifyNewVersion(silent)
              }
            }
          } else {
            __electronLog.info('Pauza: could not check for new version')
          }
        })
  ...
  async function notifyNewVersion (silent) {
    const title = await window.utils.shouldShowNotificationTitle(await window.runtime.platform(), await window.runtime.getSystemVersion()) ? 'Pauza' : ''
    const notification = new Notification(title, {
