...
const allCommands = {
  ...
  version: {
    description: 'Show current Pauza version'
  },
  logs: {
...
class Command {
  ...
  parse (input) {
    ...
    if (!this.supported[this.command]) {
      log.error(`Pauza${this.isFirstInstance ? '' : ' 2'}: command '${this.command}' is not supported`)
      return
    }
  ...
  getOpts (opts) {
    ...
    for (let i = 0; i < opts.length; i++) {
      ...
      if (!valid) {
        log.error(`Pauza${this.isFirstInstance ? '' : ' 2'}: option '${name}' is not valid for command '${this.command}'`)
      }
    }
    ...
    switch (this.command) {
      ...
      default:
        if (this.hasSupportedCommand) {
          log.info(`Pauza${this.isFirstInstance ? '' : ' 2'}: forwarding command '${this.command}' to the main instance`)
        }
    }
  }
  ...
  ver () {
    console.log(`Pauza version ${this.version}`)
  }
  ...
  cmdHelp () {
    ...
    let part = `Usage: pauza <command> ${options}\n\nCommands:`
    ...
    part = Object.keys(this.supported).reduce((acc, key) => {
      ...
      const line = `pauza ${cmds[i]}${' '.repeat(padding)} ${this.supported[key].description}`
      i++
      return `${acc}\n\t${line}`
    }, part)
